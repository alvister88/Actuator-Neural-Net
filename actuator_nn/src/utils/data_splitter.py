...
def split_data(input_file, output_dir, train_ratio=0.9, chunk_size=500, number=None, scramble=False):
    """
    ...
    scramble (bool): Whether to scramble the training data (default: False).
    """
    ...
    with open(input_file, 'r') as infile, \
        ...
        if scramble:
            # Read all lines, scramble, and write
            lines = infile.readlines()
            random.shuffle(lines)
            train_outfile.writelines(lines[:train_lines])
            val_outfile.writelines(lines[train_lines:])
        else:
            # Process in chunks without scrambling
            lines_written = 0
...
def main():
    ...
    parser.add_argument("--scramble", action="store_true", help="Scramble the training data")

    args = parser.parse_args()

    split_data(args.input_file, args.output_dir, args.train_ratio, args.chunk_size, args.number, args.scramble)
    ...
    if args.scramble:
        print("Training data has been scrambled.")
